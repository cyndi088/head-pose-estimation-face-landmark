...
def show_image(img, facepoint, bboxs, headpose):
    ...
    for faceNum in range(0, facepoint.shape[0]):
        cv2.rectangle(img,
                      (int(bboxs[faceNum, 0]), int(bboxs[faceNum, 2])),
                      (int(bboxs[faceNum, 1]), int(bboxs[faceNum, 3])),
                      (0, 0, 255),
                      2)
        for p in range(0, 3):
            plt.text(int(bboxs[faceNum, 0]),
                     int(bboxs[faceNum, 2]) - p * 30,
        ...
        for i in range(0, facepoint.shape[1] // 2):
            cv2.circle(img,
                       (int(round(facepoint[faceNum, i * 2])), int(round(facepoint[faceNum, i * 2 + 1]))),
                       1,
                       (0, 255, 0),
                       2)
    height = img.shape[0]
...
def retifyBBox(img, bbox):
    ...
    if bbox[0] < 0:
        ...
        bbox[3] = 0
    elif bbox[0] > img_width:
        bbox[0] = img_width
    elif bbox[1] > img_width:
        bbox[1] = img_width
    elif bbox[2] > img_height:
        bbox[2] = img_height
    elif bbox[3] > img_height:
        bbox[3] = img_height
    return bbox
...
def predictImage(filename):
    ...
    while line:
        ...
        level1Point = batchRecoverPart(predictpoints, bboxs, TotalSize, M_left, M_right, M_top, M_bottom, vgg_height,
                                       vgg_width)
